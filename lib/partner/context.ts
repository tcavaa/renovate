 ...
 * to its worker and a `team` account to its brigade; admin may open the portal with
 * `?store=ID` / `?worker=ID` / `?team=ID` to see exactly what that partner sees.
 * `type: null` means a role without a link — the layout shows the "not linked" card and
 * nothing else.
 */
export interface PartnerContext {
  ref: PartnerRef;
  type: 'store' | 'worker' | 'team' | null;
  name: string | null;
...
export async function loadPartnerContext(search?: { store?: string; worker?: string; team?: string }): Promise<PartnerContext | null> {
  ...
  let teamId = session.user.role === 'team' ? session.user.teamId : null;
  if (isAdmin) {
    const s = Number(search?.store);
    const w = Number(search?.worker);
    const g = Number(search?.team);
    if (Number.isInteger(s) && s > 0) storeId = s;
    else if (Number.isInteger(w) && w > 0) workerId = w;
    else if (Number.isInteger(g) && g > 0) teamId = g;
  }
  const base = { ref: { storeId: storeId ?? null, workerId: workerId ?? null, teamId: teamId ?? null }, isAdmin, userId: Number(session.user.id) };
  if (storeId) {
  ...
  if (teamId) {
    const [row] = await db.select({ nameKa: teams.nameKa, commissionRate: teams.commissionRate, email: teams.email, approvalStatus: teams.approvalStatus }).from(teams).where(eq(teams.id, teamId)).limit(1);
    return { ...base, type: 'team', name: row?.nameKa ?? `#${teamId}`, commissionRate: row?.commissionRate == null ? null : Number(row.commissionRate), email: row?.email ?? null, approvalStatus: row?.approvalStatus ?? null };
  }
...
export function partnerHref(path: string, ctx: PartnerContext): string {
  ...
  if (ctx.ref.teamId) return `${path}?team=${ctx.ref.teamId}`;
  return path;
}
