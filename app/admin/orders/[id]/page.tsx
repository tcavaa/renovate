...
export default async function AdminProjectDetailPage({
  ...
  const meta = [
    { icon: Hash, label: 'ID', value: `#${project.id}` },
    {
      ...
      value: new Date(project.createdAt).toLocaleString(dateLocale),
    },
    {
      icon: Home,
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          >
            ...
            {ka.admin.projectsList.backToAll}
          </Link>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {rooms.length === 0 && (
                <EmptyRow colSpan={6} text={ka.summary.roomsEmpty} />
              )}
            </tbody>
            {rooms.length > 0 && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-ink-muted">
                ...
                <th className="px-4 py-3">{ka.calculator.qtyAndUnit}</th>
            ...
            <tbody>
              {summary.materials.map((m) => {
                const unit = m.estimatedPriceGEL
                  ? m.qty * m.estimatedPriceGEL
                  : 0;
                return (
                  <tr
                    key={m.key}
  ...
  return (
    <Card className={highlight ? 'border-brand bg-brand/5' : undefined}>
      <CardContent className="p-5">
        ...
        <p
          className={`mt-2 font-serif text-2xl font-bold tabular-nums ${highlight ? 'text-brand-dark' : 'text-ink'}`}
        >
          {value}
        </p>
      </CardContent>
...
function Row({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4">
      ...
      <span
        className={`tabular-nums ${big ? 'font-serif text-2xl font-bold text-brand-dark' : 'font-medium'} ${muted ? 'text-ink-muted' : ''}`}
      >
        {value}
      </span>
    </div>
