...
export default function StudioPage() {
  ...
  /** "Empty the rooms" asks first — in a dialogue of ours, not the browser's. */
  ...
  const ensureSaved = useCallback(async () => (await saveDesign({ draft: true })).id, []);
      ...
      <>
        ...
        <div className="container py-20 text-center">
          ...
          <Button asChild className="mt-6">
            <Link href={designStepHref(projectId, 1)}>{t.design.addPlan}</Link>
          </Button>
        </div>
      </>
    );
      ...
      */}
      ...
      >
        ...
        <StudioTopBar
          ...
          nextHref={category === 'finishes' ? nextStepHref(projectId, 6, homeState, mode) : designStepHref(projectId, 6)}
      ...
      <Dialog open={clearOpen} onOpenChange={setClearOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogTitle className="text-center">{t.build.emptyRooms}</DialogTitle>
            <DialogDescription className="text-center">{t.build.emptyRoomsConfirm}</DialogDescription>
          </DialogHeader>
