...
import { designStepHref, designStepPosition, previousStep, previousStepHref } from '@/lib/design/steps';
...
export default function BudgetPage() {
  ...
  const { plan, styleId, mode, budgetGel, items, finishes, electrical, styleProfile, homeState, excluded, quantities, toggleExcluded, setLinesExcluded, setQuantity, clearBudgetEdits } = useDesignStore();
  const project = useProjectMeta();
  const projectId = project.id;
  // The project's calculation, when it has one: the design journey opens it beside the design.
  const calculator = useCalculatorStore();
  const [saving, setSaving] = useState(false);
  /** What the last press of "save" wrote: the button says "saved" until the design changes. */
  const [savedFor, setSavedFor] = useState<{ plan: FloorPlan; scene: unknown } | null>(null);
  ...
  const justSaved = savedFor?.plan === plan && savedFor.scene === scene;

  /**
   * Writes the design into the project's row — every time it is asked: the save button and the
   * checkout share it. An explicit save, so a project still a draft becomes a saved one.
   */
  const saveOnce = async (): Promise<number> => {
    const { id } = await saveDesign({ draft: false });
    if (project.status === 'draft') project.setStatus('saved');
    setSavedFor({ plan, scene });
    return id;
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      // The project's banner says what went wrong and offers the way out (`SaveProblemBanner`).
      useSaveProblems.getState().report('design', projectId, problemOf(e));
    } finally {
      setSaving(false);
    }
  };
  ...
  const calculated = calculator.projectId === projectId && !!calculator.homeState && calculator.rooms.length > 0 && calculator.calculated;
  ...
  return (
    <>
      ...
      <div className="container py-10 md:py-14">
          ...
          actions={
            <div className="no-print flex flex-wrap items-center gap-2">
              ...
              <Button type="button" variant="outline" onClick={save} disabled={saving}>
                ...
                {saving ? t.design.saving : justSaved ? t.design.savedTitle : t.design.saveDesign}
              </Button>
            </div>
          }
        />
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <div className="space-y-4 lg:sticky lg:top-24 lg:self-start">
            ...
            <Link href={designStepHref(projectId, 8)} className="flex h-12 items-center justify-center gap-2 rounded-[12px] border border-line bg-white text-sm font-medium text-ink hover:border-ink">
      ...
      <StepNav
        back={{ href: previousStepHref(projectId, 7, homeState, mode), label: previousStep(7, homeState, mode) === 3 ? t.design.step3 : t.design.backToStudio }}
      ...
      <CheckoutDialog open={checkoutOpen} onOpenChange={setCheckoutOpen} saveProject={saveOnce} projectId={projectId} parts={checkoutParts} onOrdered={() => project.setStatus('submitted')} />
    </>
  );
}
