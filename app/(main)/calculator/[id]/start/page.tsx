...
export default function CalculatorStep1Page() {
  ...
  const [mode, setMode] = useState<PlanMode>(() => {
    const way = search.get('way');
    if (!planOnFile && (way === 'draw' || way === 'upload')) return way;
    return plan && plan.rooms.length === 0 && plan.source === 'manual' ? 'draw' : 'upload';
  });
  ...
  const continueToPlan = () => {
    ...
    router.push(calculatorStepHref(projectId, 2));
  };
