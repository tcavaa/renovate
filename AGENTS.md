# AGENTS.md — RenovationRoom (რემონტი.ge / RenovateGE)
...
> **This file is generated from `CLAUDE.md` and is the same document under the name other
> agents look for.** Edit `CLAUDE.md` and copy it here, changing only this header — never
> edit one twin and leave the other. The last copy that drifted sat six months behind and
> described a wall system the code had already replaced.
>
> Where this file says **Claude**, it means the Anthropic API the plan parser calls
> (`lib/design/aiPlan.ts`, model `claude-opus-5`, key `ANTHROPIC_API_KEY`) — not whichever
> assistant is reading this. That is a fact about the code and does not change with the
> reader. The previous copy of this file renamed those mentions and ended up claiming that
> "Codex reads the drawing when `ANTHROPIC_API_KEY` is set", which is not what happens.
