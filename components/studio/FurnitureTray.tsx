    ...
    <div className="flex gap-2">
      ...
      */}
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        {/* Search, then every kind the shelf holds, as icons across the rest of the width. */}
        <div className="flex items-center gap-1.5">
          <div className="relative shrink-0">
            <Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-ink-faint" />
            <input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t.design.searchFurniture} aria-label={t.design.searchFurniture} className="h-7 w-28 rounded-[8px] border border-line bg-white pl-7 pr-2 text-xs text-ink placeholder:text-ink-faint focus:border-ink focus:outline-none" />
          </div>
          {/* One line that scrolls, never three that wrap: a tray whose height changes with
              the number of kinds keeps taking the canvas away. */}
          <div className="flex min-w-0 flex-1 gap-0.5 overflow-x-auto" role="radiogroup" aria-label={t.design.allKinds}>
            <button type="button" role="radio" aria-checked={kind === ''} onClick={() => setKind('')} title={t.design.allKinds} className={cn('h-7 shrink-0 rounded-[8px] px-2 text-[10px] font-semibold', kind === '' ? 'bg-ink text-white' : 'text-ink-soft hover:bg-sand-light')}>
              {t.design.allKinds}
            </button>
            {kinds.map(([k, n]) => {
              const Icon = archetypeIcon(k);
              return (
                <button key={k} type="button" role="radio" aria-checked={kind === k} onClick={() => setKind(kind === k ? '' : k)} title={`${archetypeLabel(k, locale)} · ${n}`} aria-label={archetypeLabel(k, locale)} className={cn('grid h-7 w-7 shrink-0 place-items-center rounded-[8px]', kind === k ? 'bg-ink text-white' : 'text-ink-soft hover:bg-sand-light hover:text-ink')}>
