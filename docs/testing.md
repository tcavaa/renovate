...
| `tests/unit/api/` | route helpers (envelope, `parseId`, `requireAdmin`, `handle`, rate limiting, `safeCallbackUrl`, repricing), who sees and changes a product (`productAccess`), a social sign-in's session claims (`accountClaims`, through the callbacks `auth.ts` registers), login lockout, upload byte sniffing | [auth-and-roles.md](auth-and-roles.md), [operations.md](operations.md) |
